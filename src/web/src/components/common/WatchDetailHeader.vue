<template>
  <div class="relative mb-5 flex items-start justify-between gap-4">
    <div class="min-w-0">
      <p class="mb-2 text-xs uppercase tracking-[0.24em] text-accent">
        {{ watch.isWishList ? 'Wish List' : watch.disposition ? dispositionLabel(watch) : 'Collection' }}
      </p>
    ...
    <p v-if="editSessionError" class="mb-5 text-sm text-danger">{{ editSessionError }}</p>
    <p v-if="analysisError" class="mb-5 text-sm text-danger">{{ analysisError }}</p>
  </div>
</template>
